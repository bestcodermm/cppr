...
void Netlist::ReadDelayFile()
{
  ...
  cout<<"Parsing delay file compeled!"<<endl;
  delete line_b;
}
...
void Netlist::ReadTimingFile()
{
  ...
  cout<<"quiting timing file reading"<<endl;
}
...
void Netlist::FindDataPath(Pin* Start, unordered_map<string, string> &Former )
{
  ...
  while (next != NULL){
    ...
    if (next->GetSink()->GetPinType() == 3){
      ...
      if (LatchPathMap[FileName] == NULL){
	LatchPath* pLatchPath = new LatchPath(FileName);
	LatchPathMap[FileName] = pLatchPath;}
      // else if (LatchPathMap[FileName] != NULL){
      ...
      while (current != NULL){
	DataPath.append("\t\t");
	DataPath.append(current->GetWireName());
	//current = current->GetFormerWire();
	current = WireMap[Former[current->GetWireName()]];
      }
      LatchPathMap[FileName]->AddPaths(DataPath);
...
void Netlist::FindCLKPath(Pin* Start,unordered_map<string,string> &Former)
{
  ...
  while (next != NULL){
	...
	LatchPathMap[FileName] = pLatchPath;}
      ...
      while (current != NULL){
	CLKPath.append("\t\t");
	CLKPath.append(current->GetWireName());
	current = WireMap[Former[current->GetWireName()]];}
      LatchPathMap[FileName]->AddPaths(CLKPath);
      //CLKPath<<"\n"<<endl;
      //CLKPath.close();}
    }
    else{
...
void* Netlist::CLKPathEnumeration(void *t)
{
  ...
  Former.clear();
  cout<<"CLK PAth Enumeration is done!"<<endl;
  pthread_exit((void*) 0);
}
...
void* Netlist::CalculateSlackSetup(void* t)
{
  ...
  while(!DPins_Copy.empty()){
    ...
    */
    ...
    LatchPath* DLatchPath = LatchPathMap[DPin];
    /*
    if (LatchPathMap[DPin]==NULL){
      LatchPath* pLatchPath = new LatchPath(DPin);
      LatchPathMap[DPin] = pLatchPath;}*/
    //CLKFile.open(CLKPin.c_str(), fstream::in);
    LatchPath* CLKLatchPath = LatchPathMap[CLKPin];
    
    
    if ((DLatchPath == NULL) || (CLKLatchPath == NULL)){
      ...
      outfile.append("SETUP");
      pthread_mutex_lock (&SlackMutex);
      LatchV.push_back(outfile);
      pthread_mutex_unlock (&SlackMutex);
      //output.open(outfile.c_str(),fstream::app|fstream::out);
      LatchReport* output = LatchReportMap[outfile];
      if (output == NULL){
	output = new LatchReport(outfile);  //hash table copy that?
	LatchReportMap[outfile]=output;}
	...
	cout<<"file not open!"<<endl;}
      ...
      vector<string> CLKFile = CLKLatchPath->getAllPaths();
      while (!CLKFile.empty()){
	CLKLine = CLKFile.back();
	if (CLKLine.length()==0){
	  continue;}
	...
	token3 = strtok_r(NULL,delim,&posptr);}
      ...
      vector<string> DFile = DLatchPath->getAllPaths();
	...
	//vector<Path*> LatchReportV = output->getLatchReport();
	sort(PathV.begin(),PathV.end(),ComparePostSetupPath);
	output->InitializeVector(PathV);
	//sort(LatchReportV.begin(), LatchReportV.end(),ComparePostSetupPath);
	...
	output->SetNUMPath(HowManyPath);
	output->SetPreSlackSetup(PreMin->GetPreSlackSetup());
	output->SetPostSlackSetup(PathV[0]->GetPostSlackSetup());
	//DFile.pop_back();
  ...
  cout<<"one thread is exiting!"<<endl;
  pthread_exit((void*) t);
}
  ...
  while(!DPins_Copy.empty()){
    ...
    */
    ...
    if ((DLatchPath == NULL) || (CLKLatchPath == NULL)){
      ...
      LatchReport* output = LatchReportMap[outfile];
      if (output == NULL){
	output = new LatchReport(outfile);
	LatchReportMap[outfile]=output;}
      l0_early = 0;
      l0_late = 0;
      LateCLK = "";
	...
	output->SetNUMPath(HowManyPath);
	output->SetPreSlackHold(PreMin->GetPreSlackHold());
	output->SetPostSlackHold(PathV[0]->GetPostSlackHold());
      }
...
void Netlist::GenerateSetupReport()
{
    ...
    SetupResult.open(SetupFile.c_str() , fstream::out);
    if (NUM_TEST!=0){
	...
	//File.append("SETUP");
	LatchReport* pLatchReport = LatchReportMap[File];
	SetupResult<<"setup "<<pLatchReport->GetPreSlackSetup()<<" "<<pLatchReport->GetPostSlackSetup()<<" "<<pLatchReport->GetNUMPath()<<endl;
	vector<Path*> pPaths = pLatchReport->getLatchReport();
	unsigned LIMIT = pLatchReport->GetNUMPath();
	for (unsigned w = 0; w<LIMIT; w++){
	  string CPath = pPaths[w]->GetPathName();
	  int Numoft = count(CPath.begin(),CPath.end(),'\t');
	  int NumofItem = Numoft/2+2;
	  SetupResult<<pPaths[w]->GetPreSlackSetup()<<" "<<pPaths[w]->GetPostSlackSetup()<<" "<<NumofItem<<endl;
	...
	SetupResult<<"setup "<<pLatchReport->GetPreSlackSetup()<<" "<<pLatchReport->GetPostSlackSetup()<<" "<<pLatchReport->GetNUMPath()<<endl;
	vector<Path*> pPaths = pLatchReport->getLatchReport();
	for (unsigned w = 0; w<pLatchReport->GetNUMPath(); w++){
	  string CPath = pPaths[w]->GetPathName();
	  int Numoft = count(CPath.begin(),CPath.end(),'\t');
	  int NumofItem = Numoft/2+2;
	  SetupResult<<pPaths[w]->GetPreSlackSetup()<<" "<<pPaths[w]->GetPostSlackSetup()<<" "<<NumofItem<<endl;
...
void Netlist::GenerateHoldReport()
{
    ...
    HoldResult.open(HoldFile.c_str() , fstream::out);
    if (NUM_TEST != 0){
      while ((!LatchV.empty()) && (tests<=NUM_TEST)){
	  ...
	  string File = LatchV[min];
	  LatchReport* pLatchReport = LatchReportMap[File];
	  HoldResult<<"hold "<<pLatchReport->GetPreSlackHold()<<" "<<pLatchReport->GetPostSlackHold()<<" "<<pLatchReport->GetNUMPath()<<endl;
	  vector<Path*> pPaths = pLatchReport->getLatchReport();
	  unsigned LIMIT = pLatchReport->GetNUMPath();
	  for (unsigned w = 0; w<LIMIT; w++){
	    string CPath = pPaths[w]->GetPathName();
	    int Numoft = count(CPath.begin(),CPath.end(),'\t');
	    int NumofItem = Numoft/2+2;
	    HoldResult<<pPaths[w]->GetPreSlackHold()<<" "<<pPaths[w]->GetPostSlackHold()<<" "<<NumofItem<<endl;
